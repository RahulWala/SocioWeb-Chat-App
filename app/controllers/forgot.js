...
module.exports.controllerFunction = function(app){
	...
	appRouter.post('/forgot', function(req, res, next) {
	  async.waterfall([
	    ...
	    function(token, done) {
	      sUser.findOne({ emailId: req.body.emailId }, function(err, user) {
	        if (!user) {
	...
	appRouter.get('/reset/:token', function(req, res) {
	  sUser.findOne({resetPasswordToken : req.params.token}, function(err, user) {
	...
	appRouter.post('/reset/:token', function(req, res) {
	  async.waterfall([
	    function(done) {
	      ...
	      sUser.findOne({ resetPasswordToken : req.params.token, resetPasswordExpires: { $gt: Date.now() } }, function(err, user) {
	        if (!user) {
	        	// console.log(err);
	        	req.flash('error', 'Password reset token is invalid or has expired.');
